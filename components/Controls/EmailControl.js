...
export default function EmailControl({qid, properties, errors, submission, onError, onChange, isEditable}) {
    ...
    return <div className="row" id={`id_${qid}`}>
            ...
            <div className="form-group row mb-0">
                <label htmlFor={`input_${qid}`} id={`label_${qid}`}
                    ...
                    {properties?.required === 'Yes' && (
                        <span className="text-sm text-danger ms-1">*</span>
                    )}
                </label>
                <div className="col-sm-8" id={`cid_${qid}`}>
                    <input
                        ...
                        name={`name_${qid}`}
                        id={`input_${qid}`}
                        className={`form-control email ${errors?.[qid]?.length > 0 ? 'is-invalid' : ''}`}
                    ...
                    {properties?.subLabel && (<p className="sub-label text-sm text-muted mb-0" id={`sub-label_${qid}`}>
                        {properties.subLabel}
                    </p>)}
